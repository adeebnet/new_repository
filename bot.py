...
async def on_message(message):
    ...
    if any(word.startswith('http') for word in message.content.split()): # Memeriksa apakah pesan berisi tautan
        await message.author.ban(reason="Dilarang mengirim tautan") # Melarang penulisan pesan
        await message.channel.send(f"(message.author.mention) diblokir karena mengirim tautan.") # Membuat pengumuman dalam
